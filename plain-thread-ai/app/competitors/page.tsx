...
export default function CompetitorsPage() {
  ...
  async function scrapeProfile(competitorId: string, instagramUrl: string | null) {
    if (!instagramUrl) return
    setScraping(competitorId)
    setScrapeResult(null)
    try {
      const res = await apiFetch('/api/competitors/scrape', {
        method: 'POST',
        body: JSON.stringify({ profile_url: instagramUrl, competitor_id: competitorId, max_posts: 50 }),
      })
      const data = await res.json()
      if (data.error) {
    ...
    <div className="max-w-4xl">
      ...
      {scrapeResult && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
          <p className="text-sm text-green-700">{scrapeResult}</p>
        </div>
      )}
              ...
              <div key={c.id} className="bg-white border rounded-xl overflow-hidden">
                <div className="p-4 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    {c.instagram_url && (
                      <button
                        onClick={() => scrapeProfile(c.id, c.instagram_url)}
                        disabled={scraping === c.id}
                        className="text-xs px-2 py-1 border rounded hover:border-green-300 text-green-600 disabled:opacity-50"
                      >
                        {scraping === c.id ? '⏳ Scraping...' : '📥 Scrape Posts'}
                      </button>
                    )}
                    <button
